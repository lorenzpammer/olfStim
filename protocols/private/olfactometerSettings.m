...
if strncmp(instruction,'get',3)
    ...
    % olfactometer.
   userDefinedValues = get(h.olfactometerSettings.edit,'string');
   userDefinedValues = cellfun(@(x) str2num(x),userDefinedValues); % change from a string cellarray to a vector
   
   % Write the user definable values from the gui into the
   % olfactometerInstructions structure.
   for i = 1 : length(userDefinedValues)
       % extract the index of the olfactometerInstruction field
       % corresponding to the value extracted from the gui:
       settingNumber = find([olfactometerInstructions.userSettingNumber] == i); 
       olfactometerInstructions(settingNumber).value = userDefinedValues(i);
   end
   clear settingNumber;clear userDefinedValues;
